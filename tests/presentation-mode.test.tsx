...
describe('PresentationMode contract',()=>{
 ...
 it('loads local saved decks into the meeting and fills the presentation cards',()=>{expect(client).toContain('Escolher apresentação salva');expect(client).toContain('listDecks');expect(client).toContain('renderSlideToDataUrl');expect(client).toContain('1920,1080');expect(client).toContain('object-cover');expect(client).toContain('Escolher outra');});
});