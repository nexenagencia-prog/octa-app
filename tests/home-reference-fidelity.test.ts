...
describe('reference home fidelity',()=>{
 it('uses the supplied lake hero and the requested dashboard sections',()=>{const home=read('src/app/page.tsx');expect(home).toContain('heroPhoto');expect(home).toContain('Performance geral');expect(home).toContain('GRAVAÇÕES RECENTES');expect(home).toContain('Digite sua mensagem...');expect(home).toContain('Bom dia, <b>{name}</b>')});
