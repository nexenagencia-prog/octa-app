...
const overlays = fs.readFileSync('src/components/home-hero-overlays.tsx','utf8');
for (const route of ['/agenda','/reunioes','/contatos']) assert.match(home,new RegExp(route.replaceAll('/','\\/')),`home missing ${route}`);
assert.match(overlays,/\/gravacoes/,'home hero actions must retain recordings access');
