...
export default function LoginPage(){const [email,setEmail]=useState(''); const [status,setStatus]=useState('');
 async function submit(e:FormEvent){e.preventDefault(); if(!isSupabaseConfigured){setStatus('Modo demo ativo. Configure Supabase para enviar magic links.');return;} const supabase=supabaseBrowser(); if(!supabase)return; const {error}=await supabase.auth.signInWithOtp({email,options:{emailRedirectTo:`${location.origin}/`}}); setStatus(error?error.message:'Link enviado para seu e-mail.');}
