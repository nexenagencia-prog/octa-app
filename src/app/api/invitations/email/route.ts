...
export async function POST(request:Request){
  try{
    const {email,meetingUrl,hostName}=await request.json();
    ...
    const response=await fetch('https://api.resend.com/emails',{method:'POST',headers:{Authorization:`Bearer ${apiKey}`,'Content-Type':'application/json'},body:JSON.stringify({from,to:[email],subject:'Convite para reunião no OCTA',html:`<div style="font-family:Arial,sans-serif;color:#0a2238"><h2>Você recebeu um convite para uma reunião</h2><p>${hostName?`${hostName} convidou você para uma reunião no OCTA.`:'Você foi convidado para uma reunião no OCTA.'}</p><p><a href="${meetingUrl}">Entrar na reunião</a></p></div>`})});
