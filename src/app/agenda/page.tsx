...
function brazilParts(value:Date|string){const parts=new Intl.DateTimeFormat('en-CA',{timeZone:BR_TIME_ZONE,year:'numeric',month:'2-digit',day:'2-digit'}).formatToParts(new Date(value));const get=(type:Intl.DateTimeFormatPartTypes)=>Number(parts.find(part=>part.type===type)?.value);return{year:get('year'),month:get('month'),day:get('day')}}
function brazilDateKey(value:Date|string){const{year,month,day}=brazilParts(value);return toDateKey(year,month-1,day)}
function capitalize(value:string){return value.charAt(0).toUpperCase()+value.slice(1)}

function AgendaContent(){
 const search=useSearchParams();
 const now=useMemo(()=>new Date(),[]);
 const today=useMemo(()=>brazilParts(now),[now]);
 const initialKey=toDateKey(today.year,today.month-1,today.day);
 const[cursor,setCursor]=useState({year:today.year,month:today.month-1});
 ...
 const monthLabel=useMemo(()=>capitalize(new Intl.DateTimeFormat('pt-BR',{month:'long',year:'numeric',timeZone:BR_TIME_ZONE}).format(new Date(Date.UTC(cursor.year,cursor.month,1,12)))),[cursor]);
 const selectedDate=useMemo(()=>{const[y,m,d]=selectedKey.split('-').map(Number);return new Date(Date.UTC(y,m-1,d,12))},[selectedKey]);
 const dayLabel=new Intl.DateTimeFormat('pt-BR',{day:'numeric',month:'long',year:'numeric',timeZone:BR_TIME_ZONE}).format(selectedDate);
 const demoForDay=useMemo(()=>demoMeetings.filter(meeting=>brazilDateKey(meeting.scheduledAt)===selectedKey),[selectedKey]);
