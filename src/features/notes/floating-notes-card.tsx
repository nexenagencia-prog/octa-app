...
export function FloatingNotesCard({roomSlug,meetingTitle,onClose}:{roomSlug?:string;meetingTitle?:string;onClose:()=>void}){
  ...
  return <div ref={panelRef} className={`floatingReferenceNote ${minimized?'isMinimized':''} ${expanded?'isExpanded':''}`} style={expanded?undefined:{left:position.x,top:position.y}}>
    ...
    <style jsx>{`
      .floatingReferenceNote{position:fixed;z-index:10000;width:min(420px,calc(100vw - 24px));height:360px;min-width:300px;min-height:240px;padding:11px 10px 10px;border-radius:23px;border:1px solid rgba(255,255,255,.28);background:linear-gradient(135deg,rgba(112,101,92,.46) 0%,rgba(55,50,46,.51) 43%,rgba(10,12,13,.84) 100%);box-shadow:inset 0 1px 0 rgba(255,255,255,.12),0 26px 70px rgba(0,0,0,.44);backdrop-filter:blur(34px) saturate(118%);-webkit-backdrop-filter:blur(34px) saturate(118%);overflow:hidden;color:#f5f3f1;font-family:-apple-system,BlinkMacSystemFont,"SF Pro Display","SF Pro Text",Inter,sans-serif;resize:both}
