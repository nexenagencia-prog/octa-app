...
type Props={initialMessages:ChatMessage[];onClose:()=>void;onActivity?:()=>void;roomId?:string;userId?:string;userName?:string;hidden?:boolean;onIncoming?:()=>void;onCountChange?:(count:number)=>void};

export function ChatOverlay({initialMessages,onClose,onActivity,roomId='demo',userId='u-host',userName='Você',hidden=false,onIncoming,onCountChange}:Props){
 ...
 useEffect(()=>{
  const receive=(event:Event)=>{const detail=(event as CustomEvent<MeetingChatEvent>).detail;if(!detail||detail.roomId!==roomId)return;setMessages(current=>current.some(message=>message.id===detail.id)?current:[...current,detail]);onIncoming?.();onActivity?.()};
  window.addEventListener('octa-chat-message',receive);return()=>window.removeEventListener('octa-chat-message',receive);
 },[onActivity,onIncoming,roomId]);
 function send(e:FormEvent){e.preventDefault();const text=body.trim();if(!text)return;const message:MeetingChatEvent={id:crypto.randomUUID(),roomId,userId,userName,body:text,createdAt:new Date().toISOString()};setMessages(m=>[...m,message]);window.dispatchEvent(new CustomEvent('octa-chat-send',{detail:message}));setBody('');setEmojiOpen(false);onActivity?.()}
