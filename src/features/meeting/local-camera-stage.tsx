...
export function LocalCameraStage({cameraEnabled,micEnabled,photoUrl,fallbackPhotoUrl}:Props){
 ...
 return <div className="relative size-full bg-black"><video ref={videoRef} autoPlay playsInline muted className="size-full object-cover" style={{transform:'scaleX(-1)'}}/>{status==='loading'&&<div className="absolute inset-0 grid place-items-center bg-black/35 text-xs text-white/50">Ativando câmera…</div>}</div>;
}
