...
function Sidebar(){
  return <aside className="hidden w-[280px] shrink-0 flex-col border-r border-white/[.06] bg-black p-4 xl:flex">
...
export function InstantMeetingClient({slug,title,onAddParticipant}:{slug:string;title:string;onAddParticipant?:()=>void}){
  ...
  return <main className="relative h-[100dvh] overflow-hidden bg-[#020202] text-white">
    <div className="flex h-full min-w-0"><Sidebar/>
      <section className="instant-stage relative flex min-w-0 flex-1 flex-col overflow-hidden bg-[#020202] px-4 py-4 lg:px-6">
        <header className="flex items-center justify-between gap-4"><div className="flex items-center gap-3 rounded-[22px] border border-white/10 bg-white/[.025] px-4 py-2.5"><span className="size-2.5 rounded-full bg-emerald-300 shadow-[0_0_16px_#38f5c4]"/><div><div className="text-sm font-medium">{title}</div><div className="mt-0.5 text-[10px] uppercase tracking-[.16em] text-emerald-300">AO VIVO <span className="ml-2 text-white/35">· 9:16</span></div></div><ChevronDown size={15} className="text-white/45"/></div><div className="flex items-center gap-2">{onAddParticipant&&<button onClick={onAddParticipant} className="meeting-silver-card flex h-10 items-center gap-2 rounded-full border px-4 text-xs text-white"><UserPlus size={14}/> Adicionar participante</button>}<button onClick={copyLink} className="flex h-10 items-center gap-2 rounded-full border border-white/10 bg-white/[.03] px-4 text-xs text-white/80"><Copy size={14}/> Copiar link</button><button className="grid size-10 place-items-center rounded-full border border-white/10 bg-white/[.03]"><MoreHorizontal size={17}/></button></div></header>
        <div className="no-scrollbar mt-3 grid min-h-0 flex-1 grid-cols-[minmax(0,1fr)] gap-4 overflow-y-auto 2xl:grid-cols-[minmax(0,1fr)_360px]">
          <div className="flex min-h-0 flex-col items-center justify-center">
            <div className="instant-video-frame relative w-full max-w-[390px] overflow-hidden rounded-[32px] border border-white/10 bg-[#070708] shadow-[0_30px_90px_rgba(0,0,0,.56)]" style={{aspectRatio:'9/16'}}>
              <div className="absolute inset-0" style={{filter:filterCss(filter,intensity)}}>{isLiveKitConfigured?<LiveKitStage room={slug} identity={currentUser.id} name={currentUser.displayName}/>:<><Image src={focus.avatarUrl!} alt={focus.displayName} fill priority className="object-cover"/><div className="absolute inset-0 bg-gradient-to-t from-black/90 via-transparent to-black/30"/></>}</div>
              ...
              <div className="absolute right-[-66px] top-1/2 z-20 hidden -translate-y-1/2 flex-col gap-3 lg:flex"><button className="flex size-11 flex-col items-center justify-center rounded-full border border-white/10 bg-[#09090a] text-white/80"><Heart size={17}/><span className="mt-0.5 text-[8px]">128</span></button><button onClick={()=>setChat(true)} className="flex size-11 flex-col items-center justify-center rounded-full border border-white/10 bg-[#09090a] text-white/80"><MessageCircle size={17}/><span className="mt-0.5 text-[8px]">32</span></button><button onClick={share} className="grid size-11 place-items-center rounded-full border border-white/10 bg-[#09090a] text-white/80"><Send size={17}/></button></div>
              {chat&&<ChatOverlay initialMessages={demoMessages} onClose={()=>setChat(false)}/>} 
            </div>
            <div className="instant-thumb-strip mt-3 flex w-full max-w-[590px] items-center gap-2 overflow-x-auto rounded-[18px] border border-white/[.06] bg-white/[.025] p-2 no-scrollbar">{people.map(p=><button key={p.id} onClick={()=>setFocused(p.id)} className={`relative shrink-0 rounded-[15px] border p-1 ${focused===p.id?'border-white/55 bg-white/10':'border-white/8 bg-white/[.025]'}`}><Image src={p.avatarUrl!} alt={p.displayName} width={52} height={52} className="size-11 rounded-[11px] object-cover"/><span className="absolute bottom-0 right-0 size-2.5 rounded-full border-2 border-[#080809] bg-emerald-300"/></button>)}<span className="grid size-12 shrink-0 place-items-center rounded-[14px] border border-white/8 text-xs text-white/55">+3</span></div>
            <div className="instant-controls mt-3 flex w-full max-w-[900px] items-center justify-center gap-1 rounded-[24px] border border-white/[.07] bg-white/[.03] p-2 backdrop-blur-xl">
    ...
    {toast&&<div className="fixed left-1/2 top-5 z-[90] -translate-x-1/2 rounded-full border border-white/10 bg-black/90 px-4 py-2 text-xs shadow-2xl backdrop-blur-xl">{toast}</div>}
  </main>;
}
