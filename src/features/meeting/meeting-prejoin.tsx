 ...
 return <main className="min-h-[100dvh] bg-[#070707] px-4 py-6 text-white md:px-8">
  <div className="mx-auto grid min-h-[calc(100dvh-48px)] max-w-[1180px] items-center gap-8 lg:grid-cols-[minmax(320px,430px)_1fr]">
   <section className="mx-auto w-full max-w-[430px]"><div className="relative w-full overflow-hidden rounded-[34px] border border-white/10 bg-[#141414] shadow-[0_34px_100px_rgba(0,0,0,.55)]" style={{aspectRatio:'9/16'}}>
    <video ref={videoRef} autoPlay playsInline muted className={`absolute inset-0 h-full w-full object-cover transition-opacity duration-200 ${cameraEnabled&&status==='ready'?'opacity-100':'opacity-0'}`} style={{transform:'scaleX(-1)'}}/>
    ...
    <div className="absolute left-4 top-4 rounded-full border border-white/10 bg-black/35 px-3 py-1.5 text-[10px] font-semibold tracking-[.12em] text-white/70 backdrop-blur-xl">PRÉVIA</div>
   ...
   <section className="mx-auto w-full max-w-[590px]"><div className="text-[11px] font-semibold uppercase tracking-[.18em] text-white/35">Antes de entrar</div><h1 className="mt-3 text-3xl font-semibold tracking-[-.04em] md:text-5xl">{title}</h1><p className="mt-4 max-w-xl text-sm leading-6 text-white/50">Veja sua imagem antes de entrar. A câmera do Mac ou computador mantém a proporção original e a OCTA mostra somente o recorte vertical, sem esticar, achatar ou deformar.</p>
    ...
    <input ref={fileRef} className="hidden" type="file" accept="image/*" onChange={event=>choosePhoto(event.target.files?.[0])}/><button type="button" aria-label={photoActionLabel} onClick={openPhotoPicker} className="mt-3 flex w-full items-center justify-between rounded-[22px] border border-white/10 bg-white/[.04] p-4 text-left transition hover:bg-white/[.07]"><span><b className="block text-sm">{photoActionLabel}</b><small className="mt-1 block text-white/45">A foto é escolhida pelo usuário e aparece quando a câmera estiver oculta, sempre com recorte proporcional.</small></span><ImagePlus size={19}/></button>
