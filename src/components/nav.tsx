...
export function DashboardSidebar({collapsed=false,onToggle}:{collapsed?:boolean;onToggle?:()=>void}){
  ...
  return <><aside className={`octa-sidebar hidden xl:flex ${collapsed?'is-collapsed':''}`}>
    <div className="octa-side-brand"><OctaLogo/></div>
...
export function TopNav(){const path=usePathname();const router=useRouter();const[query,setQuery]=useState('');const[profile,setProfile]=useState<EditableProfile>(defaultEditableProfile);useEffect(()=>{setProfile(getProfile());const onUpdate=(e:Event)=>setProfile((e as CustomEvent<EditableProfile>).detail??getProfile());window.addEventListener(PROFILE_UPDATED_EVENT,onUpdate);return()=>window.removeEventListener(PROFILE_UPDATED_EVENT,onUpdate)},[]);const runSearch=(e:React.FormEvent)=>{e.preventDefault();if(query.trim())router.push(`/reunioes?q=${encodeURIComponent(query.trim())}`)};return <header className="octa-topbar"><OctaLogo/><nav className="hidden items-center gap-8 lg:flex">{topItems.map(({href,label})=><Link key={label} href={href} data-cms-id={`global.top.link.${label.toLowerCase().replace(/[^a-z0-9]+/g,'-')}`} data-cms-type="text" className={`octa-toplink ${path===href?'is-active':''}`}>{label}</Link>)}</nav><div className="octa-top-actions"><ThemeToggle/><form onSubmit={runSearch} className="octa-search hidden min-w-[340px] xl:flex" data-cms-id="global.top.search" data-cms-type="container"><Search size={21}/><input value={query} onChange={e=>setQuery(e.target.value)} aria-label="Buscar" placeholder="Buscar reunião, pessoa ou gravação"/></form><div className="octa-top-right"><NotificationsCenter/><Link href="/profile" className="octa-profile-pill hidden xl:flex"><img src={profile.avatarUrl} alt=""/><span>{profile.displayName}</span><ChevronRight size={13}/></Link><Link href="/profile" className="grid size-10 place-items-center rounded-full bg-white/70 text-[#102944] xl:hidden"><CircleUserRound size={18}/></Link></div></div></header>}
