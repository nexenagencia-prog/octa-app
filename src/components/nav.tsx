...
export function DashboardSidebar({collapsed=false,onToggle}:{collapsed?:boolean;onToggle?:()=>void}){
  ...
  return <><aside className={`octa-sidebar hidden xl:flex ${collapsed?'is-collapsed':''}`}>
    <div className="octa-profile flex items-center gap-3 px-5 pt-6" data-cms-id="global.sidebar.profile" data-cms-type="container">
      <div className="relative size-12 shrink-0 overflow-hidden rounded-full border border-white/30 bg-white/15"><img src={profile.avatarUrl} alt={profile.displayName} className="h-full w-full object-cover"/><span className="absolute bottom-0 right-0 size-3 rounded-full border-2 border-black bg-emerald-400"/></div>
...
export function TopNav(){
  const path=usePathname();const router=useRouter();const[query,setQuery]=useState('');const[notificationsOpen,setNotificationsOpen]=useState(false);
  const runSearch=(e:React.FormEvent)=>{e.preventDefault();if(query.trim())router.push(`/reunioes?q=${encodeURIComponent(query.trim())}`)};
  const notifications=[
    {icon:CalendarDays,title:'Reunião em 30 minutos',text:'Planejamento de Marketing começa às 14:30.',time:'agora'},
    {icon:Video,title:'Gravação processada',text:'A gravação da última reunião já está disponível.',time:'há 8 min'},
    {icon:Sparkles,title:'OCTA Skills atualizou',text:'Seu desempenho semanal subiu 6,4%.',time:'há 22 min'},
  ];
  return <header className="octa-topbar"><OctaLogo/><nav className="hidden items-center gap-8 lg:flex">{topItems.map(({href,label})=><Link key={label} href={href} data-cms-id={`global.top.link.${label.toLowerCase().replace(/[^a-z0-9]+/g,'-')}`} data-cms-type="text" className={`octa-toplink ${path===href?'is-active':''}`}>{label}</Link>)}</nav><div className="ml-auto flex min-w-0 items-center gap-4"><ThemeToggle/><form onSubmit={runSearch} className="octa-search hidden min-w-[340px] xl:flex" data-cms-id="global.top.search" data-cms-type="container"><Search size={21}/><input value={query} onChange={e=>setQuery(e.target.value)} aria-label="Buscar" placeholder="Buscar reunião, pessoa ou gravação"/></form><div className="octa-notification-wrap"><button type="button" className="octa-notification-button" aria-label="Notificações" aria-expanded={notificationsOpen} onClick={()=>setNotificationsOpen(open=>!open)}><Bell size={22}/><span className="octa-notification-badge">3</span></button>{notificationsOpen&&<div className="octa-notification-panel"><header><strong>Notificações</strong><span>3 novas</span></header>{notifications.map(({icon:Icon,title,text,time})=><div key={title} className="octa-notification-item"><span className="octa-notification-icon"><Icon size={16}/></span><div><b>{title}</b><p>{text}</p><small>{time}</small></div></div>)}</div>}</div><Link href="/profile" className="grid size-10 place-items-center rounded-full bg-white/70 text-[#102944] lg:hidden"><CircleUserRound size={18}/></Link></div></header>
}
