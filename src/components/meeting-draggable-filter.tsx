...
export function MeetingDraggableFilter({selected,intensity,onSelect,onIntensity,onClose}:{selected:string;intensity:number;onSelect:(id:string)=>void;onIntensity:(value:number)=>void;onClose:()=>void}){
  ...
  useEffect(()=>{
    ...
    const height=300;
    setPosition({x:Math.max(16,(window.innerWidth-width)/2),y:Math.max(16,(window.innerHeight-height)/2)});
  },[]);
  useEffect(()=>{
  ...
  return <div ref={panelRef} className="meeting-filter-overlay is-positioned" style={{left:position.x,top:position.y}}>
    <div className="meeting-filter-drag-handle" onPointerDown={e=>{const rect=panelRef.current?.getBoundingClientRect();if(!rect)return;drag.current={offsetX:e.clientX-rect.left,offsetY:e.clientY-rect.top};}} aria-label="Mover filtros" title="Arraste para mover"><GripHorizontal size={18}/></div>
