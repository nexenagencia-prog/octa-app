...
export function ToolOverlay(){
  const {tool,closeTool}=useToolOverlay();const [filter,setFilter]=useState('natural');const [intensity,setIntensity]=useState(80);
  const panelRef=useRef<HTMLDivElement|null>(null);const drag=useRef<{offsetX:number;offsetY:number}|null>(null);const [position,setPosition]=useState({x:0,y:112});
  useEffect(()=>{if(tool==='calculator')setPosition({x:Math.max(16,window.innerWidth-458),y:112})},[tool]);
  ...
  if(tool==='filters')return <div className="octa-floating-tool" data-tool="filters"><VideoFilterPanel selected={filter} intensity={intensity} onSelect={setFilter} onIntensity={setIntensity} onClose={closeTool}/></div>;
  return <div ref={panelRef} className="octa-floating-tool is-positioned" data-tool="calculator" style={{left:position.x,top:position.y}}><div className="relative"><div className="calculator-drag-handle" onPointerDown={e=>{const rect=panelRef.current?.getBoundingClientRect();if(!rect)return;drag.current={offsetX:e.clientX-rect.left,offsetY:e.clientY-rect.top};}} aria-label="Mover calculadora" title="Arraste para mover"><GripHorizontal size={18}/></div><button onClick={closeTool} aria-label="Fechar calculadora" className="floating-tool-close"><X size={15}/></button><CalculatorPanel/></div></div>;
}
