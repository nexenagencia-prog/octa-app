...
export function AppShell({children}:{children:React.ReactNode}){
  const [collapsed,setCollapsed]=useState(true);const [compact,setCompact]=useState(false);
  useEffect(()=>{const sync=()=>{try{const saved=localStorage.getItem(STORAGE_KEY);setCollapsed(saved===null?true:saved==='1');setCompact(localStorage.getItem('octa-compact-mode')==='1')}catch{}};sync();window.addEventListener('octa-preferences-updated',sync);return()=>window.removeEventListener('octa-preferences-updated',sync)},[]);
