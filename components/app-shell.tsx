...
export function AppShell({children,variant='default'}:{children:React.ReactNode;variant?:'dashboard'|'default'}){
  ...
  return <ToolOverlayProvider><main className={`octa-page ${collapsed?'sidebar-collapsed':''} ${compact?'is-compact':''}`}>
    <DashboardSidebar collapsed={collapsed} onToggle={toggle} dashboard={variant==='dashboard'}/>
    <div className={`octa-main ${variant==='dashboard'?'octa-main-dashboard':''}`}>{variant==='default'&&<TopNav/>}<div className="octa-app-body">{children}</div></div>
    <MobileNav/>
